...
def render_glyph_8x16(font, cp):
    ...
    # Scale to fit CELL_W wide and ASCENT tall (keep aspect ratio)
    scale_x = CELL_W / glyph_w
    scale_y = ASCENT / glyph_h
    scale   = min(scale_x, scale_y)

    new_w = max(1, round(glyph_w * scale))
    ...
    # Build 8x16 cell: glyph centered horizontally, top-aligned in ascent area
...
def make_bdf(font_path, codepoint_ranges, out_bdf_path):
    ...
    for start, end in codepoint_ranges:
        for cp in range(start, end + 1):
            try:
                rows = render_glyph_8x16(font, cp)
                if rows is not None:
...
def build_group(group_name):
    ...
    bin_path = SRC_DIR / f"{out_name}.bin"
...
def main():
    ...
    for group_name, group in FONT_GROUPS.items():
        p = SRC_DIR / f"{group['out_name']}.bin"
        if p.exists():
