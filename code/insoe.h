...
void handleUpdateRoot() {
    ...
    html += F("<p class=\"empty\">Only rupertwriter.bin, hwalja_*.bin font files, and initial.png can be uploaded here.</p><section class=\"grid\"><div class=\"card\"><h2>Firmware Update</h2><label for=\"fwFile\">rupertwriter.bin</label><input type=\"file\" id=\"fwFile\" accept=\".bin\"><button id=\"fwBtn\" onclick=\"uploadFirmware()\">Upload and Update</button><p class=\"status\" id=\"fwStatus\">Waiting...</p></div><div class=\"card\"><h2>Resource Upload</h2><label for=\"resTarget\">Target</label><select id=\"resTarget\"><option value=\"hwalja_hangul.bin\">hwalja_hangul.bin</option><option value=\"hwalja_jamo.bin\">hwalja_jamo.bin</option><option value=\"hwalja_jp.bin\">hwalja_jp.bin</option><option value=\"hwalja_greek_cyrillic.bin\">hwalja_greek_cyrillic.bin</option><option value=\"hwalja_arabic.bin\">hwalja_arabic.bin</option><option value=\"hwalja_indic.bin\">hwalja_indic.bin</option><option value=\"hwalja_sea.bin\">hwalja_sea.bin</option><option value=\"hwalja_misc.bin\">hwalja_misc.bin</option><option value=\"initial.png\">initial.png</option></select><label for=\"resFile\">File</label><input type=\"file\" id=\"resFile\" accept=\".bin,.png\"><button id=\"resBtn\" onclick=\"uploadResource()\">Upload to SD</button><p class=\"status\" id=\"resStatus\">Waiting...</p></div></section></main><script>async function sendUpload(inputId,statusId,buttonId,targetName){const input=document.getElementById(inputId);const status=document.getElementById(statusId);const btn=document.getElementById(buttonId);if(!input.files.length){alert('Select a file.');return;}const file=input.files[0];if(targetName&&file.name!==targetName){alert('File name must be '+targetName+'.');return;}btn.disabled=true;status.textContent='Uploading...';const fd=new FormData();fd.append('file',file,targetName||file.name);try{const res=await fetch('/update',{method:'POST',body:fd});const text=await res.text();status.textContent=res.ok?(text||'Done.'):('Failed: '+res.status+' '+text);}catch(e){status.textContent='Error: '+e.message;}finally{btn.disabled=false;}}function uploadFirmware(){sendUpload('fwFile','fwStatus','fwBtn','rupertwriter.bin');}function uploadResource(){sendUpload('resFile','resStatus','resBtn',document.getElementById('resTarget').value);}</script></body></html>");
    server.send(200, "text/html", html);
}
