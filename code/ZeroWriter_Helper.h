...
void printCleanText(U8G2_FOR_ADAFRUIT_GFX &u8g2, const String& text, int x, int y, bool isMenu = false, int maxWidth = 800) {
    ...
    for (int i = 0; i < len; ) {
        ...
        if (c_val >= 0x80) {
            if ((c_val & 0xE0) == 0xC0) l = 2;      // 2바이트 (유럽어 라틴 특수문자 등)
        ...
        // --- [본문 / 메뉴 / 상태바 전 영역 서체 통합 엔진] ---
        if (l == 3) {
            // 1. 한글 (3바이트) -> 사장님 전용 둥근모 서체
            // 한글 폰트 상태 번호를 '1'로 지정
            if (currentFontState != 1) { u8g2.setFont(Typewriter_16px); currentFontState = 1; }
            
            u8g2.drawUTF8(cx, y, buf);
            cx += 16 + (isMenu ? 1 : letterSpacing);
        } else if (l == 1) {
            // 2. 일반 영어/기호 (1바이트) -> 유니폰트 적용!
            // 유니폰트 상태 번호를 '2'로 지정해서 한글과 절대 겹치지 않게...
            if (currentFontState != 2) { u8g2.setFont(u8g2_font_unifont_te); currentFontState = 2; }
            
            u8g2.drawUTF8(cx, y, buf);
            cx += 8 + (isMenu ? 1 : 0);
        } else {
            // 3. 유럽어 라틴 특수문자 (2바이트 이상) -> 유니폰트
            // 🚀 이것도 유니폰트이므로 똑같이 상태 번호 '2'!
            if (currentFontState != 2) { u8g2.setFont(u8g2_font_unifont_te); currentFontState = 2; }
            
            u8g2.drawUTF8(cx, y, buf);
            u8g2.drawUTF8(cx + 1, y, buf); 
